...
describe("createRetryAttempt", () => {
  ...
  beforeEach(() => {
    ...
    db.prepare(
      `INSERT INTO jobs (id, identity_hash, repository_key, pr_number, head_sha, source_mode, policy_hash, state, version)
       VALUES ('job-fail', 'hash-fail', 'pba-webapp', 7, ?, 'registered-source', 'ph', 'failed', 2)`,
    ).run("c".repeat(40));
    db.prepare(
  ...
  it("requeues failed job and returns new run id", () => {
    const runId = createRetryAttempt(db, "job-fail");
    expect(runId).toBeTruthy();

    const job = db
      .prepare(`SELECT state FROM jobs WHERE id = 'job-fail'`)
      .get() as { state: string };
